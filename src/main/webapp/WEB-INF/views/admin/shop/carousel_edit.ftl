...
<script type="text/javascript">
    require(["modules/mdtable/init",'admin/shop/carousel/edit/init'], function(Table,CarouselEdit){
...
<div class="modal-dialog">
    <div class="modal-content">
	    <div class="form-horizontal" role="form">
	        <div class="form-body">
	        	<input type="hidden" value="${(inbaId)!}">
                ...
                <div class="modal-body">
                	...
                	<div class="form-group">
	                   	<div class="table-responsive paging-table sort-table">
							<input type="hidden" name="path" value="admin/shop/carousel/goodsInformation/list?inbaId=${(selectId)!}" /><!-- 分页、排序、查询用到的path -->
							<table class="table table-striped table-hover"><!-- 统一风格写法 -->
							 ...
							 <#if list??>
							 	<#list list as good>
							 		<tr class="gooditem">
							 			<td ><span class="goodid" style="display:none">${(good["goodId"]!)}</span><center value="${(good["gopiUrl"])!}"><img src="${(good["gopiUrl"])!}" style="height:60px;width:60px;"/></center></td>
										
										<td ><center value="${(good["goodName"])!}">${(good["goodName"])!}</center></td>
										
							 			<td ><center>${(good["goodPrice"]!)}</center></td>
							 		
										<#if goodId?? && goodId == good["goodId"]>
